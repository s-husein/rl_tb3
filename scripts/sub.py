...
if __name__ == '__main__':
    ...
    while True:
        ...
        # cv_img = CvBridge.compressed_imgmsg_to_cv2(ros_img.data)

        data = ros_img.data[12:]
        cv_img = cv.imdecode(np.frombuffer(data, np.uint8), cv.IMREAD_UNCHANGED)

        # cv_img = CvBridge().imgmsg_to_cv2(ros_img.data)


        # print(ros_img.deserialize())

    #     print(compressed.header)

    #     ros_arr = np.frombuffer(ros_img.data, np.uint8)
    #     cv_img = cv.imdecode(ros_arr, 1)
        
    #     print(ros_arr.shape)
        # cv_img = CvBridge().compressed_imgmsg_to_cv2(ros_img, desired_encoding='bgr8')
        cv_img = cv_img/4000.0
